...
function loadGoods() {
	$.each(goodsList, function(i, item) {
		var goodsHtml = '<div class="goods-item">' +
			...
			'<img class="goods-image" src="' + item.imgUrl + '" style="width: 150px; height: 150px;" />' +
			'<span id="goods-info">' +
			item.goodsInfo +
			'</span>' +
			'</div>' +
			'<div class="col-md-2 car-goods-info goods-params">' + item.goodsParams + '</div>' +
			...
			'<button type="button" class="btn btn-primary item-delete">Remove</button>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'</div>'
